...
fn start() {
    let mut html = File::open("resources/test/index.html").unwrap();
    let mut html_contents = String::new();
    html.read_to_string(&mut html_contents).unwrap();
    let window = Window::with_handler(Handler);
    window.title("Hello, world!");
    window.load(&html_contents);

    window.eval(JS.to_owned());
}

struct Handler;

impl webview_api::Handler for Handler {
    fn handle(&mut self, _: Window, msg: &str) {
        println!("{}", msg);
    }
}
...
fn main() {
    unsafe { webview_api::start(start) }
}
